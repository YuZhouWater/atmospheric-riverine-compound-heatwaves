# ============================================================
# This script performs riverine heatwave detection for 796 rivers.
...
# 1) Read data
# -----------------------------
# Expect: first column is date (t), other columns are temperature series (rivers)
data1 <- read_csv("data/Temp_WT.csv")

# Standardize first column name to "t"
names(data1)[1] <- "t"

# Convert wide -> long: columns [2:n] become different "river"
plotdata <- data1 %>%
  pivot_longer(
    cols = -t,
    names_to = "river",
...
Rhw_annual_frequency      <- tibble(year = years)
Rhw_annual_duration       <- tibble(year = years)
Rhw_annual_mean_intensity <- tibble(year = years)

rivers <- unique(plotdata$river)

# -----------------------------
# 4) Main loop over rivers
# -----------------------------
for (r in rivers) {
  ...
  Rhw_annual_mean_intensity[[r]] <- yearly_int_full$anom_mean

  # 4.3 Annual duration (days): allocate event duration across years
  dur_year <- process_heatwave_events(Rhw$event, years)
  Rhw_annual_duration[[r]] <- dur_year$duration
}

# -----------------------------
# 5) Save outputs
# -----------------------------
write_csv(Rhw_annual_frequency,      file.path(outdir, "RHW_annual_frequency.csv"))
write_csv(Rhw_annual_duration,       file.path(outdir, "RHW_annual_duration.csv"))
write_csv(Rhw_annual_mean_intensity, file.path(outdir, "RHW_annual_mean_intensity.csv"))
